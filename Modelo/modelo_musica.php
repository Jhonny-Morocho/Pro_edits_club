<?php 
	...
	@$url_directorio_edit=$_POST['url_directorio_edit'];
	@$nombre_arhivo_url_inicio=$_POST['nombre-arhivo-url-inicio'];
...
/*		$respuesta=array('post'=> $_POST , 
	...
	if(@$_POST['editar_cancion']=='edicion-cancion'){
		...
		if(isset($musica_url)){
			...
			//Renombar nombre de archivo
			$archivoAbierto = fopen('../biblioteca/'.$nombre_arhivo_url_inicio, 'r');//

			fclose($archivoAbierto); 	

			rename( '../biblioteca/'.$nombre_arhivo_url_inicio, '../biblioteca/'.$url_directorio_edit );
			...
			$stmt=$conn->prepare("UPDATE productos SET
				...
				if ($stmt->affected_rows) {
					...
					'id_insertado'=>$id_insertado,
					'id_producto'=>$id_producto,
					'nombre_arhivo_edit'=>$url_directorio_edit,
	...
	if(@$_POST['registro']=='eliminar'){
		...
		try{
			...
			//=====================BORRAR ARHIVO CON PHP======================//
			$titulo_archivo=$_POST['titulo_arhivo'];
			$dir='../biblioteca/'.$titulo_archivo; //puedes usar dobles comillas si quieres 
			$result_servidor="";
			if(file_exists($dir)) 
			{ 
			if(unlink($dir)) 
			$result_servidor="El archivo fue borrado"; 
			} 
			else 
			$result_servidor="Este archivo no existe"; 
			...
			$estado=0;
				...
				if ($stmt->affected_rows) {
					...
					'result_servidor'=>$result_servidor,
