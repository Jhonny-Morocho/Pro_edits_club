<?php 
	...
	if(@$_POST['registro-nuevo']=='nuevo'){
		...
		try {
			...
			$stmt=$conn->prepare("INSERT INTO proveedor (nombre,apellido,apodo,correo,tipo_usuario,password, estado) VALUES(?,?,?,?,?,?,?)");

			$stmt->bind_param("sssssss",$nombre,$apellido,$apodo,$email,$tipo_usuario,
				$password_hashed,$estado);
	...
	//////////////////LOGIN DE AMINISTRADOR O DJ///////////////////
		...
		try {
			...
			$stmt=$conn->prepare("SELECT *FROM proveedor WHERE correo=? and estado ='1' ");
			...
			$stmt->bind_result(
								...
								$editado,$estado);// para consultas regresa el numeor de columas importante ponerlo
	...
	if(@$_POST['registro']=='eliminar'){
		...
		try {
			$stmt=$conn->prepare("UPDATE proveedor SET estado=? WHERE id=? ");

			$stmt->bind_param('ss',$estado,$id_borrar);
